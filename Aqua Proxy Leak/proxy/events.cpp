...
void proxypack() {
    Dialog a;
    a.addLabelWithIcon("`#meow `9Commands", 1796, LABEL_SMALL);
    a.addSpacer(SPACER_SMALL);
    a.addCustom("add_url_button||`cProxy Discord Server``|NOFLAGS|https://discord.gg/t9MwutGcaf");
    a.addTextBox("`9Credits: `#! vaccat#7777");
    a.addSpacer(SPACER_SMALL);
    a.addTextBox("`cCommands:");
    a.addTextBox("`2/proxy `9(Commands)|left");
    a.addSpacer(SPACER_SMALL);
    a.addTextBox("`4Pro `9Commands:");
    a.addTextBox("`2/find [item name] `9(Item Finder)");
    a.addSpacer(SPACER_SMALL);
    ...
    a.addTextBox("`2/spam `9(Enable Auto Spam)");
    a.addTextBox("`2/setspam `5[Text] `9(Set Spam Text)");
    a.addSpacer(SPACER_SMALL);
    ...
    a.addSpacer(SPACER_SMALL);
    a.addTextBox("`1Dropping Commands:");
    ...
    a.addTextBox("`2/wl `9(Drop all `9World Locks)|left");
    a.addTextBox("`2/dl `9(Drop all `1Diamond Locks)|left");
    a.addTextBox("`2/bgl `9(Drop all `eBlue Gem Locks)|left");
    a.addSpacer(SPACER_SMALL);
...
bool events::out::generictext(std::string packet) {
    ...
    if (var.get(0).m_key == "action" && var.get(0).m_value == "input") {
        ...
        /*else if (find_command(chat, "wr")) {
        ...
        else if (find_command(chat, "cd ") || find_command(chat, "cdrop ")) {
            // retardation
            string cdropcount = chat.substr(3);
            if (packet.find("cdrop ")) {
                int cdropcount = packet.find("cdrop " + 6);
            } else {
                int cdropcount = packet.find("cd " + 3);
            }
            int wl = stoi(cdropcount) % 100;
            ...
            gt::send_log("`9Dropping `c" + to_string(wl) + " `9Wls & `c" + to_string(dl) + " `9Dls.");
            return true;
        }
...
bool events::in::variantlist(gameupdatepacket_t* packet) {
    ...
    case fnv32("OnConsoleMessage"): {
        ...
        varlist[1] = "`9[`5! vaccat#7777```9]`` " + varlist[1].get_string();
        if (spam) {
